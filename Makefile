...
HTTPS_GIT := https://github.com/NextNet-Works/xnet-chain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=xnet \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(XNET_BINARY) \
...
build-docker:
	...
	$(DOCKER) create --name xnet -t -i ${DOCKER_IMAGE}:latest xnet
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp xnet:/usr/bin/xnetd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/xnet/xnet"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(XNET_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/xnet:Z xnet/node "./xnetd testnet init-files --v 4 -o /xnet --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\xnetd:/xnet\Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)\build\node1\xnetd:/xnet\Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)\build\node2\xnetd:/xnet\Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)\build\node3\xnetd:/xnet\Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
else
	@docker run --rm -v $(CURDIR)/build/node0/xnetd:/xnet:Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)/build/node1/xnetd:/xnet:Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)/build/node2/xnetd:/xnet:Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
	@docker run --rm -v $(CURDIR)/build/node3/xnetd:/xnet:Z xnet/node "./xnetd tendermint unsafe-reset-all --home=/xnet"
endif
...
PACKAGE_NAME:=github.com/xnetfoundation/xnet
